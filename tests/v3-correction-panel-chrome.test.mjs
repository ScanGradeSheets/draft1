...
test('correction entry is drawn on the sheet and uses a number-only custom keypad', () => {
  ...
  assert.match(source, /\.on-sheet-correction-focus\s*\{[^}]*border:\s*0 solid transparent/s)
  assert.match(source, /\.on-sheet-correction-focus\s*\{[^}]*outline:\s*2px solid rgba\(36,\s*90,\s*164,\s*0\.72\)/s)
  assert.match(source, /\.on-sheet-correction-focus\s*\{[^}]*outline-offset:\s*3px/s)
  ...
  assert.match(source, /\.on-sheet-correction-focus--committing\s*\{[^}]*animation:\s*correction-focus-release 180ms ease-out forwards/s)
  assert.match(source, /@keyframes correction-focus-release/)
