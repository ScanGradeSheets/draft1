...
test('manifest installs ScanGrade as a standalone education app with the approved logo', () => {
  ...
  const expected = new Map([
    ['icons/scangrade-icon-192-v2.png', 192],
    ['icons/scangrade-icon-512-v2.png', 512],
    ['icons/scangrade-icon-maskable-512-v2.png', 512],
  ])
  ...
  assert.deepEqual(
    pngDimensions('public/icons/apple-touch-icon-180-v2.png'),
...
test('document advertises the manifest, Apple icon, standalone mode and app title', () => {
  ...
  assert.match(html, /rel="apple-touch-icon" sizes="180x180" href="%BASE_URL%icons\/apple-touch-icon-180-v2\.png"/)
...
test('Apple Home Screen icon is a full-resolution PNG with a versioned path', () => {
  const icon = pngRgba('public/icons/apple-touch-icon-180-v2.png')
  assert.equal(icon.width, 180)
  assert.equal(icon.height, 180)
  assert.ok(icon.bytes.length > 8_000)
})

test('Apple Home Screen icon has a pure-white edge and optical ink centering', async () => {
  const image = await loadImage(fileURLToPath(new URL(
    'public/icons/apple-touch-icon-180-v2.png',
    rootUrl,
  )))
  ...
  assert.ok(Math.abs(weightedX / inkWeight - image.width / 2) < 1)
