...
test('legacy capture compaction is explicitly scoped away from modern layouts', async () => {
  ...
  assert.match(appSource, /\.scan-grade--legacy-student\.scan-grade--landing\s*\{[^}]*justify-content:\s*flex-start;[^}]*padding:\s*72px 10px 0;[^}]*overflow:\s*hidden;/s)
  ...
  assert.match(appSource, /\.scan-grade--legacy-capture \.student-scan-bar\s*\{[^}]*height:\s*50px;[^}]*margin:\s*4px auto 12px;/s)
